...
RUN echo "\e[1;32mUpdating packages and installing SFTP server package...\e[0m" && \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get -o Dpkg::Options::='--force-confold' -fuy --allow-downgrades --allow-remove-essential --allow-change-held-packages install ssh fuse && \
